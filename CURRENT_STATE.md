...
| Date | Title | Summary |
|---|---|---|
| 2026-05-13 | Multi-user + parallel GPU shipped (Phases A, C, E, F0–F2) | Supabase JWT auth + per-user data isolation (`uploads.user_id`). File→RTSP shim (mediamtx + ffmpeg). NVStreamer + DeepStream REST integration; `nvmultiurisrcbin` + `[streammux] drop-pipeline-eos=1 live-source=1`. `asyncio.Semaphore(MAX_CONCURRENT_STREAMS=2)` for concurrent jobs. Events routed by `meta["sensorId"]`. Hard-cap-with-events now marks `completed` not `failed`. Frontend signed-in chip + 401→`/login?expired=1`. `LEGACY_OWNER_USER_ID` env backfills NULL `user_id` rows. Single AWS GPU host (g6.xlarge L4) — split deploy merged. |
| 2026-05-12 | [F2 diff plan](docs/state/log/2026-05-12-f2-diff-plan.md), [F1 spike](docs/state/log/2026-05-12-f1-source-tagging-spike.md), [F0 diff plan](docs/state/log/2026-05-12-f0-diff-plan.md), [NVStreamer rediagnosis](docs/state/log/2026-05-12-nvstreamer-rediagnosis.md) | Path-B (DS REST + mediamtx) chosen + executed. NVStreamer 3.1.0 file-streamer ruled definitively broken (stripped libav); RTSP path works. |
