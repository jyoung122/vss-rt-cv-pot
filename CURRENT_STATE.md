...
## 2026-05-04 — upload queue + swappable VLM provider seam landed

Two architectural changes shipped in five atomic commits each (see `git log --oneline`):

**Upload job queue (commits `687ed7f` → `3d1614d`).** `POST /api/upload` no longer touches Redis, `current_stream_url.txt`, or the Docker socket inline. It saves the file, ffprobes, inserts the `uploads` row, then `enqueue()`s into a new in-process `asyncio.Queue` ([`backend/app/upload_queue.py`](backend/app/upload_queue.py)). A serial worker task spawned in the FastAPI lifespan owns the side-effects: sets `current_video_id`, writes the URI file, restarts `vss-rt-cv`, waits for ingest plateau (3 consecutive identical `event_count` polls, armed after `min(15s, duration_s)`, hard cap `duration_s + 30s`), then advances. Exceptions are logged and swallowed so the worker keeps draining. Queue is depth-capped at `UPLOAD_QUEUE_MAX_DEPTH` (default 10) — over-capacity uploads get a 503 with `{"error":"queue full"}`. Concurrent uploaders now serialize cleanly instead of racing on the global `current_video_id` key (the previous failure mode silently dropped all but the last upload's perception). `GET /api/uploads/:id/progress` extended with `queue_status` (`"queued" | "active" | "done" | null`) and `queue_position` (0-indexed). Frontend hook gained a conditional 6th leading "Queued" pill in the progress strip — only shown when `queue_status="queued"` at any point — with `"N ahead — waiting for DeepStream"` sub-text and 2 s poll cadence (vs 1 s for ingest). 21 backend tests still pass + 7 new for the queue. **This is a workaround, not the cure.** The proper fix is multi-source via `nvstreammux` so the per-upload restart goes away entirely; logged as priority architectural debt in [`docs/v1/plan.md`](docs/v1/plan.md).

**VLM provider seam (commits `2095b07` → `da2ba56`).** `vlm_validator.py` now reads `VLM_PROVIDER=cosmos|openai` (default `cosmos`) and delegates to one of two isolated provider classes in [`backend/app/vlm_providers/`](backend/app/vlm_providers/). `cosmos.py` is the existing NIM path with the model id env-driven (`COSMOS_MODEL`, default `nvidia/cosmos-reason2-2b`) so the planned 2b→8b swap is also a no-code change. `openai_provider.py` is new — extracts JPEG frames from the existing mp4 clip via ffmpeg (`fps=VLM_FRAME_FPS,scale=768:-2,-q:v 4`) into a `tempfile.TemporaryDirectory()` so frames don't pollute `/data/incidents/`, sends them as base64 `image_url` content parts to `chat.completions.create` with `response_format=json_object` and `temperature=0.1`. Default model `gpt-5.4-mini`. Optional `OPENAI_BASE_URL` lets it point at any OAI-compatible endpoint (vLLM, Ollama, Together, etc.). Shared `prompts.py` and `parsing.py` mean both providers use the same prompt strings and verdict parser; module isolation is enforced (cosmos doesn't import openai and vice versa, asserted by tests). `vlm_model` column captures which model produced each verdict — mixed history is fine. `cosmos` compose service moved under `profiles: [gpu]` so plain `docker compose up` no longer pulls the 30 GB NIM image; bring it up with `docker compose --profile gpu up -d`. New env vars (`VLM_PROVIDER`, `OPENAI_API_KEY`, `OPENAI_MODEL`, `OPENAI_BASE_URL`, `COSMOS_MODEL`, `VLM_FRAME_FPS`) documented in `.env.example`. 10 new tests pass, total 31/31.
...
**Not yet validated.** Live OpenAI call against `gpt-5.4-mini` hasn't been made — tests stub `AsyncOpenAI`. First deploy with `VLM_PROVIDER=openai` should run analyze on `91_Country_Club-1777844748` (the open Cosmos-2B aftermath case) and inspect the verdict + `vlm_model` column. Listed under "VLM provider follow-ups" in [`docs/v1/plan.md`](docs/v1/plan.md).
...
**Endpoints.** `POST /api/uploads` (multipart; saves file, ffprobes, inserts uploads row, **enqueues** via `upload_queue.py` — the worker owns `current_video_id` / `current_stream_url.txt` / `vss-rt-cv` restart. Response includes `queue_status` and `queue_position`; 503 with `{"error":"queue full"}` above `UPLOAD_QUEUE_MAX_DEPTH`). `GET /api/uploads` (list with `event_count` / `track_count` join). `GET /api/uploads/:id`. **`GET /api/uploads/:id/progress`** (single SQL aggregate returning `{video_id, duration_s, event_count, incidents_total, vlm_pending, vlm_done, vlm_skipped, vlm_error, vlm_enabled, queue_status, queue_position}`; powers the upload-progress strip). `DELETE /api/uploads/:id` (cascade drops events). `GET /api/uploads/:id/events?group=tracks|none` (track-summary or raw rows). `GET /api/uploads/:id/incidents` (rule-detected + VLM-annotated incidents). `POST /api/uploads/:id/analyze` (run rule pack + VLM validator if `VLM_ENABLED=true`; returns `incidents_found`). `GET /api/uploads/:id/playback` (FileResponse off disk). `WS /ws/events` (live broadcast of mdx-raw entries). `GET /healthz` and `/health` for Docker healthcheck.
