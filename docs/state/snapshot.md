...
**Upload queue (`backend/app/upload_queue.py`).** In-process `asyncio.Queue` + semaphore-bounded concurrent fan-out (default `MAX_CONCURRENT_STREAMS=2`). Each `_process_job` coroutine owns its own publisher / sensor / plateau watcher / teardown. Per-job: `rtsp_publisher.start` (ffmpeg `-re` → mediamtx) → `wait_until_publishing` (poll mediamtx HTTP API for `sourceReady`) → `nvstreamer.register_sensor` → `deepstream.add_stream` (POST to `vss-rt-cv:9000`). Plateau detection: 3 consecutive identical `event_count` polls, armed after `min(15s, duration_s)`, hard cap `duration_s + 60s`. Cap with non-zero events ⇒ `dss_status='completed'`; cap with zero events ⇒ `'failed'`. Teardown (always, `finally`): `deepstream.remove_stream` → `nvstreamer.unregister_sensor` → `rtsp_publisher.stop` → `sem.release()` → `q.task_done()`. Caps depth at `UPLOAD_QUEUE_MAX_DEPTH` (default 10) → 503 with `{"error":"queue full"}`.

**Event indexer ([`backend/app/event_indexer.py`](../../backend/app/event_indexer.py)).** Background asyncio task spawned at lifespan start. Uses Redis `XREADGROUP` (group `indexer`, consumer `indexer-1`, MKSTREAM), parses pipe-delimited objects out of each `metadata` JSON envelope, **routes each frame to its `video_id` via `meta["sensorId"]`** (set by `nvmsgconv` from the `camera_id` we passed to `/api/v1/stream/add`), looks up `fps` from the `uploads` row (cached per-process) to compute `t_seconds = frame_id / fps`, and `executemany`s the rows. `INSERT ... ON CONFLICT (video_id, frame_id, track_id) DO NOTHING` collapses duplicates. Orphan frames (unknown sensorId, or sensorId without a matching uploads row) are dropped + counted. Poison-pill safe: ack on success or on parse error, retry the outer loop on connection errors.
...
- **NVStreamer 3.1.0 file-streamer adapter broken (upstream).** The image ships `launch_vst` against libav60 but ~30 FFmpeg runtime libs are stripped; `POST /api/v1/file` fails. **Bypassed 2026-05-12:** uploads land on disk, then `rtsp_publisher.py` wraps them as RTSP via mediamtx, and NVStreamer is invoked through `POST /api/v1/sensor/add` (RTSP path uses GStreamer DESCRIBE, no libav). See [log/2026-05-12-nvstreamer-rediagnosis.md](log/2026-05-12-nvstreamer-rediagnosis.md).
...
- **Runbook punch list (uncovered 2026-05-03 during fresh-VM rerun, not yet fixed):**
  ...
  - [`docs/v1/deploy/deploy.md`](../v1/deploy/deploy.md) seed-clip wording: clicking a seeded row in the UI doesn't trigger the pipeline. The seed flow inserts the DB row + copies the file but never enqueues to `upload_queue`. Only `POST /api/upload` triggers ingest. Rewrite Step 7 to say "upload via drag-drop or curl" or add a "Start pipeline" button on the detail page.

---

## Architectural debt (priority)

- ✅ **Resolved 2026-05-12** — Multi-source DeepStream + per-event `sensorId` tagging shipped. `nvmultiurisrcbin` + REST :9000, asyncio.Semaphore for concurrency, docker socket mount removed. See [log/2026-05-12-f2-diff-plan.md](log/2026-05-12-f2-diff-plan.md).
