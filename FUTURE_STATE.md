...
| NVStreamer 3.1.0 `RTSP DESCRIBE` returns 404; logs show `Container: , videoCodec:` empty and "Codec format not supported" | NVStreamer 3.1.0 metadata discovery is broken. Affects every file format we tried (mp4 ✓, h264 ✓, with/without audio, baseline profile, +faststart). | Bypass NVStreamer entirely. Source `type=3 uri=file:///...` reads files directly via uridecodebin. |
